                    ...
                    <div class="card-content">
                        <div class="content">
                            <div class="field">
                                ...
                                <div class="control">
                                    <CmbRegional :id_prop="currentUser.id" :sel="unidade.id_regional"
...
export default {
    ...
    methods: {
        loadData() {
            this.isLoading = true;
            unidadeService.getUnidade(this.unidade.id_unidade).then(
                (response) => {
                    let data = response.data;
                    this.unidade.codigo = data.codigo;
                    this.unidade.nome = data.nome;
                    this.unidade.tipo = data.tipo;
                    this.unidade.id_regional = data.id_regional;
                },
                (error) => {
                    this.message =
                        (error.response &&
        ...
        edit() {
            ...
            if (!this.v$.$error) {
                ...
                unidadeService.edit(this.unidade).then(
                    (response) => {
                        this.showMessage = true;
    ...
    mounted() {
        ...
        this.unidade.id_unidade = this.$route.params.id;
        if (cUser) {
            if (cUser.id != 11) {
                this.message = "Você não tem permissão para alterar unidades";
                this.showMessage = true;
                this.type = "alert";
                this.caption = "Unidade";
                setTimeout(() => {
